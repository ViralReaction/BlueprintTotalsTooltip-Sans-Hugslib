	...
	static class ConstructibleUtility
	{
		public static List<ThingDefCount> MaterialsNeededSafe(this IConstructible constructible)
		{
			...
			if (constructible is Frame frame)
				return new List<ThingDefCount>(constructible.TotalMaterialCost().Select(x => new ThingDefCount(x.thingDef, frame.ThingCountNeeded(x.thingDef))).Where(x => x.Count > 0));
